...
export default function Settings() {
  ...
  const handleSaveAlgorithm = async () => {
    ...
    try {
      ...
      fetchLogs();
    } catch { showMsg("Failed to save", "error"); } finally { setLoading(false); }
  };
  ...
  const handleBackup = async () => {
    try {
      ...
      fetchLogs();
    } catch { showMsg("Backup failed", "error"); } finally { setLoading(false); }
  };
  ...
  const handleClearCache = async () => {
    try {
      ...
      fetchLogs();
    } catch { showMsg("Failed to clear cache", "error"); } finally { setLoading(false); }
  };
  ...
  const handleResetSettings = async () => {
    ...
    try {
      ...
      fetchLogs();
    } catch { showMsg("Reset failed", "error"); } finally { setLoading(false); }
  };
        ...
        <div className="settings-content-area">
          ...
          {activeTab === "registration" && (
            <>
              ...
              <div className="toggle-group">
                ...
                <ToggleRow label="Employer Registration" value={employerRegistrationEnabled} onChange={setEmployerRegistrationEnabled} />
          ...
          {activeTab === "employers" && (
            <>
              ...
              <p className="settings-subtext">Review employer accounts waiting for approval.</p>
              <div className="logs-viewer">
                {pendingEmployers.length === 0 ? (
                  <div className="empty-state">
                    <p>No pending employers at the moment.</p>
                  </div>
                ) : (
          ...
          {activeTab === "users" && (
            <>
              ...
              <p className="settings-subtext">View and manage registered users.</p>
              ...
              <div className="logs-viewer" style={{ maxHeight: "400px", overflowY: "auto" }}>
                    ...
                    <tbody>
                      {filteredUsers.map(user => (
                        <tr key={user._id}>
                          ...
                          <td>{user.email}</td>
                          <td><span style={{textTransform: "capitalize", fontSize: "13px", color: "#64748b"}}>{user.role || "user"}</span></td>
                          <td>
                            <span className={`status-badge ${user.status === "active" ? "active" : user.status === "suspended" ? "pending" : "rejected"}`}>
                              {user.status || "active"}
                            </span>
                          </td>
                        </tr>
