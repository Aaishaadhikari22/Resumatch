...
export default function EmployerLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      saveAuth("employer", res.data.token, employerInfo);
      
      if (res.data.employer.status === "rejected") {
      ...
      if (error.response?.data?.requireOTP) {
         setRequireOTP(true);
         setEmployerId(error.response.data.employerId);
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      setRequireOTP(false);
      setOtp("");
      // Now attempt login again automatically
  ...
  const handleSendOTP = async (e) => {
    ...
    setLoading(true);
    try {
      const resp = await API.post("/auth/employer/forgot-password", { email: forgotEmail });
      if (resp.data) {
        setOtpSent(true);
  ...
  const handleResetPassword = async (e) => {
    ...
    setLoading(true);
    try {
      await API.post("/auth/employer/reset-password", { email: forgotEmail, otp, newPassword });
      setOtpSent(false);
