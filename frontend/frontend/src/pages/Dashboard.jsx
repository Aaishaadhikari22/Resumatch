...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([fetchStats(), fetchLogs(), fetchPendingVerifications()]).finally(() => setLoading(false));
  }, []);
  ...
  const fetchPendingVerifications = async () => {
    try {
      const empRes = await API.get("/admin/employers/pending").catch(() => ({ data: [] }));
      const jobRes = await API.get("/admin/jobs/pending").catch(() => ({ data: [] }));
      const total = (empRes.data?.length || 0) + (jobRes.data?.length || 0);
  ...
  const chartData = [
    ...
    { name: "Feb", users: 600, jobs: 380 },
    { name: "Mar", users: 800, jobs: 520 },
    { name: "Apr", users: 1000, jobs: 760 },
    { name: "May", users: 1200, jobs: 950 }
  ];
        ...
        <>
          ...
          <div className="quick-actions-container">
            ...
            <div className="quick-actions-grid">
              ...
              <button className="quick-action-btn warning" onClick={() => navigate("/verification")}>
          ...
          <div className="dashboard-main-flex">
            ...
            <div className="side-panel flex-1">
              <div className="action-required-card">
                ...
                <div className="urgent-item" onClick={() => navigate("/verification")}>
                   <span className="urgent-label">Pending Employers</span>
                   <span className="urgent-badge">{stats.pendingEmployers || 0}</span>
                </div>
